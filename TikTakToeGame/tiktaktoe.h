...
public:
  ...
  bool winnerCheck(int row, int col, PlayingPiece *piece) {
    ...
    for (int i = 0; i < size; i++) {
      if (gameBoard->getPiece(row, i).getType() == piece->getType()) {
        count++;
      }
    }
    if (count == size) {
    ...
    count = 0;
    for (int i = 0; i < size; i++) {
      if (gameBoard->getPiece(i, col).getType() == piece->getType()) {
        count++;
      }
    }
    if (count == size) {
    ...
    count = 0;
    for (int i = 0; i < size; i++) {
      if (gameBoard->getPiece(i, i).getType() == piece->getType()) {
        count++;
      }
    }
    if (count == size) {
    ...
    count = 0;
    for (int i = 0; i < size; i++) {
      if (gameBoard->getPiece(i, size - 1 - i).getType() == piece->getType()) {
      }
    }
    if (count == size) {
      return true;
